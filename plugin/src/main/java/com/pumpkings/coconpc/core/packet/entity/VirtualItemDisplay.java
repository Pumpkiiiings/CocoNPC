...
public class VirtualItemDisplay extends VirtualEntity {
    ...
    public void updateMetadata(Player player) {
        ...
        int off = com.pumpkings.coconpc.util.ServerVersionInfo.DISPLAY_OFFSET;
        metadata.add(new EntityData<>(11 + off, EntityDataTypes.VECTOR3F, new Vector3f(translation.x, translation.y, translation.z)));
        metadata.add(new EntityData<>(12 + off, EntityDataTypes.VECTOR3F, new Vector3f(scale.x, scale.y, scale.z)));
        metadata.add(new EntityData<>(13 + off, EntityDataTypes.QUATERNION, new Quaternion4f(leftRotation.x, leftRotation.y, leftRotation.z, leftRotation.w)));
        metadata.add(new EntityData<>(14 + off, EntityDataTypes.QUATERNION, new Quaternion4f(rightRotation.x, rightRotation.y, rightRotation.z, rightRotation.w)));
        metadata.add(new EntityData<>(18 + off, EntityDataTypes.FLOAT, shadowRadius));
        metadata.add(new EntityData<>(19 + off, EntityDataTypes.FLOAT, shadowStrength));
        if (item != null) {
            metadata.add(new EntityData<>(23 + off, EntityDataTypes.ITEMSTACK, SpigotConversionUtil.fromBukkitItemStack(item)));
        }
        metadata.add(new EntityData<>(24 + off, EntityDataTypes.BYTE, toDisplayTransformByte(transform)));
