...
public enum Message {
    ...
    EDITOR_HINT("<green>Right Click (or 'F' key) to switch Modes (Rotate/Move). <red>Shift + Left Click to exit."),
    ...
    MODE_ROTATION_PRIMARY("{prefix}<yellow>Mode: <gold>Primary Rotation (Whole Limb) 🔄 <gray>| Scroll: Yaw | Shift+Scroll: Pitch"),
    MODE_ROTATION_ROLL("{prefix}<yellow>Mode: <gold>Side Tilt / Roll (Whole Limb) 🔃 <gray>| Scroll: Tilt Left/Right"),
    MODE_POSITION_VERTICAL("{prefix}<yellow>Mode: <green>Move Vertical/Depth ↕️ <gray>| Scroll: Up/Down | Shift+Scroll: Front/Back"),
    MODE_POSITION_HORIZONTAL("{prefix}<yellow>Mode: <green>Move Horizontal ↔️ <gray>| Scroll: Left/Right"),
    MODE_ROTATION_JOINT_PRIMARY("{prefix}<yellow>Mode: <light_purple>Joint Bending (Elbow/Knee Only) 🦴 <gray>| Scroll: Yaw | Shift+Scroll: Bend"),
    MODE_ROTATION_JOINT_ROLL("{prefix}<yellow>Mode: <light_purple>Joint Tilt (Elbow/Knee Only) 🦴 <gray>| Scroll: Tilt Joint"),
    ...
    ACTION_INVALID_TYPE("{prefix}<red>Invalid action type: <yellow>{type}<red>. Valid types: message, player, console, chat, broadcast, title, subtitle, give_exp, take_exp, connect, cooldown, require_permission, require_money"),
    ...
    public String get(CocoNPC plugin, String... replacements) {
        String message = plugin.getConfigManager().messages.getString(this.name(), defaultMessage);
        if (this != PREFIX) {
            String prefix = plugin.getConfigManager().messages.getString(PREFIX.name(), PREFIX.getDefault());
